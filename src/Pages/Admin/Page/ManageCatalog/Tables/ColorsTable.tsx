...
export const columns = (
        ...
        onCheckedChange={(value) => {
          ...
          setSelectedData((prev) => {
            if (prev.includes(row.original)) {
              return prev.filter(
                (item) => item.color_id !== row.original.color_id
              );
            } else {
  ...
  },
  {
    ...
    header: "Color Name",
    cell: ({ row }) => (
      <div className="capitalize">{row.getValue("color_name")}</div>
    ),
  },
];
export function ColorsTable() {
  ...
  const handleEditColor = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setIsLoading(true);
    await axios
        ...
        <div className="flex gap-2 px-2 py-1.5 w-full border rounded-md">
          ...
          <button disabled={selectedData.length !== 1}>
            <Edit
              onClick={() => {
                setActionName("edit");
                setInputValue(selectedData[0].color_name);
              }}
              className={`${
