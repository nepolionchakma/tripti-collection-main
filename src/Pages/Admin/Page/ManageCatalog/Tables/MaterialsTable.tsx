...
export const columns = (
        ...
        onCheckedChange={(value) => {
          ...
          setSelectedData((prev) => {
            if (prev.includes(row.original)) {
              return prev.filter(
                (item) => item.material_id !== row.original.material_id
              );
            } else {
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="capitalize">{row.getValue("material_name")}</div>
    ),
  },
];
      ...
      {actionName === "add" ? (
        <CustomModal className="w-[40%] scrollbar-thin">
          <div className="flex items-center justify-between bg-amber-300 py-0.5 px-1 sticky top-0">
            <h1 className="font-semibold">Add Material</h1>
        ...
        actionName === "edit" && (
          <CustomModal className="w-[40%] scrollbar-thin">
            <div className="flex items-center justify-between bg-amber-300 py-0.5 px-1 sticky top-0">
              <h1 className="font-semibold">Edit Material</h1>
              <X onClick={handleCloseModal} className="cursor-pointer" />
            </div>
            <form
              action=""
        ...
        <div className="flex gap-2 px-2 py-1.5 w-full border rounded-md">
          ...
          <button disabled={selectedData.length !== 1}>
            <Edit
              onClick={() => {
                setActionName("edit");
                setInputValue(selectedData[0].material_name);
              }}
              className={`${
